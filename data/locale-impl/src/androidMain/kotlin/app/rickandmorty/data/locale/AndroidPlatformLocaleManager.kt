...
public class AndroidPlatformLocaleManager(
  ...
  private val localeUpdateMutex = Mutex()
  private val compatLocaleChanges = MutableSharedFlow<Locale?>(replay = 1)
  ...
  override fun getAppLocale(): Flow<Locale?> {
    ...
    return merge(initialLocale, updates).distinctUntilChanged()
  }

  override suspend fun setAppLocale(locale: Locale?) {
    localeUpdateMutex.withLock {
      val localeList =
      ...
      if (Build.VERSION.SDK_INT < 33) {
        compatLocaleChanges.emit(locale)
      }
    }
  }
...
private fun LocaleListCompat.toLocale(): Locale? {
  return if (isEmpty) null else Locale(get(0)!!.toLanguageTag())
}
